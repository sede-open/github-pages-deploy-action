...
function deploy(action) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            yield execute_1.execute(`git branch -D ${action.branch} --quiet`, action.workspace, action.silent);
            core_1.info(`Removing ${temporaryDeploymentBranch}`);
            yield execute_1.execute(`git branch -D ${temporaryDeploymentBranch} --quiet`, action.workspace, action.silent);
