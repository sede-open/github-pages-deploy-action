...
function deploy(action) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            core_1.info(`Resetting the branch???`);
            yield execute_1.execute(`git checkout -b ${process.env.GITHUB_SHA}`, action.workspace, action.silent);
            // TODO: Move this...
            if (action.singleCommit) {
                core_1.info(`Removing ${temporaryDeploymentBranch}`);
                yield execute_1.execute(`git branch -D ${temporaryDeploymentBranch} --force`, action.workspace, action.silent);
            }
            else {
                core_1.info(`Removing ${action.branch}`);
