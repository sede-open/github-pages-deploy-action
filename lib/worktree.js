...
function generateWorktree(action, worktreedir, branchExists) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (branchExists) {
                yield execute_1.execute(`git fetch --no-recurse-submodules --depth=1 ${action.repositoryName ? action.repositoryPath : 'origin'} ${action.branch}`, action.workspace, action.silent);
            }
            ...
            if (branchExists) {
                ...
                checkout.commitish = `${action.repositoryName
