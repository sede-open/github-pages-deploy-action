    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            if (typeof action.sshKey === 'string') {
                ...
                const agent = child_process_1.execFileSync('ssh-agent');
                agent
                    .toString()
                    .split('\n')
                    .map((line) => {
                    const agentVariables = /^(SSH_AGENT_PID|SSH_AUTH_SOCK)=(.*); export \1/.exec(agent[line]);
                    if (agentVariables && agentVariables.length) {
                        core_1.exportVariable(agentVariables[1], agentVariables[2]);
                    }
                });
                // Adds the SSH key to the agent.
                action.sshKey.split(/(?=-----BEGIN)/).map((line) => __awaiter(this, void 0, void 0, function* () {
                    yield execute_1.execute(`ssh-add - ${line.trim()}\n`, sshDirectory, action.silent);
                }));
                yield execute_1.execute(`ssh-add -l`, sshDirectory, action.silent);
            }
            else {
