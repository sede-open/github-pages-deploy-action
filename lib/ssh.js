    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            if (typeof action.sshKey === 'string') {
                ...
                const agentOutput = yield execute_1.execute(`ssh-agent`, sshDirectory, action.silent);
                const lines = agentOutput.toString().split('\n');
                for (const line in lines) {
                    const matches = /^(SSH_AUTH_SOCK|SSH_AGENT_PID)=(.*); export \1/.exec(lines[line]);
                    if (matches && matches.length > 0) {
                        core_1.exportVariable(matches[1], matches[2]);
                    }
                }
                ...
                child_process_1.default.execFileSync('ssh-agent -l');
            }
            else {
